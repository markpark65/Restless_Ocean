...
{
	{ "초롱 아귀",		150, 180, 20, 24, 95, 105, AttributeType::Luminous,
		"\n- 패시브    : 공격력이 지속적으로 증가한다."
		"\n- 일반 공격 : 몸통 박치기로 공격한다."
		"\n- 특수 공격 : 빛을 축적해 다음 공격을 강화한다."
	, MapType::BuildingMap },

	{ "바이퍼 피쉬",	120, 150, 16, 20, 110, 120, AttributeType::Hidden,
		"\n- 패시브    : 속도가 증가하며 기습 공격을 한다."
		"\n- 일반 공격 : 물어뜯기로 공격한다." 
		"\n- 특수 공격 : 연속 공격으로 추가 피해를 입힌다.",
	MapType::SeaCaveMap },

	{ "대왕 오징어",	180, 210, 18, 22, 80, 90, AttributeType::Giant,
		"\n- 패시브    : 체력을 회복한다."
		"\n- 일반 공격 : 먹물을 발사해 공격한다."
...
{
	{ "왕관 해파리",	400, 450,  24, 28, 95, 105, AttributeType::Luminous,
		"\n- 패시브    : 강한 빛으로 적을 실명시켜 다음 턴 피해를 무효화한다."
		"\n- 일반 공격 : 촉수 공격 후 독 피해를 추가로 입힌다."
		"\n- 특수 공격 : 빛을 모아 폭발시켜 큰 피해를 입힌다.",
		MapType::BuildingMap },

	{ "흉내 문어",		350, 400, 20, 24, 120, 130, AttributeType::Hidden,
...
Monster* MonsterFactory::GenerateMonster(int level, int battleCount, AttributeType mapType)
{
	...
	//출현지 문제 해결
	if (mapType == AttributeType::Luminous) stat.map = MapType::SeaCaveMap;
	else if (mapType == AttributeType::Hidden) stat.map = MapType::BuildingMap;
	else if (mapType == AttributeType::Giant) stat.map = MapType::CollapsedShipMap;

	float hpStat = 1.0f + (level - 1) * 0.2f;
	float attackStat = 1.0f + (level - 1) * 0.1f;

	stat.health = static_cast<int>(Random::getRandomValue(monsterTemplate->minHealth, monsterTemplate->maxHealth) * hpStat);
	stat.attack = static_cast<int>(Random::getRandomValue(monsterTemplate->minAttack, monsterTemplate->maxAttack) * attackStat);
